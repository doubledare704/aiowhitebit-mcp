...
async def run_server(args):
    ...
    if args.whitebit_api_key and args.whitebit_api_secret:
        exchanges[ExchangeType.WHITEBIT] = {"api_key": args.whitebit_api_key, "api_secret": args.whitebit_api_secret}

    # Add Binance exchange if credentials are provided
    if args.binance_api_key and args.binance_api_secret:
        exchanges[ExchangeType.BINANCE] = {"api_key": args.binance_api_key, "api_secret": args.binance_api_secret}
    ...
    # Run forever
    try:
        logger.info("Multi-exchange MCP server running")
...
def parse_args():
    ...
    parser.add_argument("--name", default="Multi-Exchange MCP", help="Name of the MCP server")
