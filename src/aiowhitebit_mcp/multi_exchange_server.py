...
class MultiExchangeMCP:
    ...
    def __init__(
        ...
        if exchanges:
            for exchange_type, credentials in exchanges.items():
                self._add_exchange(exchange_type, credentials.get("api_key"), credentials.get("api_secret"))
        else:
    ...
    def _add_exchange(
        self, exchange_type: ExchangeType, api_key: Optional[str] = None, api_secret: Optional[str] = None
    ) -> None:
        ...
        self.exchanges[exchange_type] = create_exchange(
            exchange_type=exchange_type, api_key=api_key, api_secret=api_secret
        )
    ...
    def _register_tools(self):
        ...
        async def get_kline(
            ...
            """
            exchange_type = exchange.exchange if exchange else None
            result = await self._get_exchange(exchange_type).get_kline(market.market, interval, start_time, end_time)
        ...
        async def cancel_order(
            ...
            """
            exchange_type = exchange.exchange if exchange else None
            result = await self._get_exchange(exchange_type).cancel_order(order_id, market.market)
            return {"order": result}
        ...
        async def get_order_status(
            ...
            """
            exchange_type = exchange.exchange if exchange else None
            result = await self._get_exchange(exchange_type).get_order_status(order_id, market.market)
            return {"order": result}
        ...
        async def get_active_orders(market: MarketPair, exchange: Optional[ExchangeParams] = None) -> Dict:
            ...
            """
            exchange_type = exchange.exchange if exchange else None
            result = await self._get_exchange(exchange_type).get_active_orders(market.market)
