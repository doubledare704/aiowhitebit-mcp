...
from aiowhitebit_mcp.cache import get_cache, cached, clear_cache, get_all_caches
...
async def test_cache():
    ...
    assert result1["call_count"] == result2["call_count"], f"Call count should be the same: {result1['call_count']} != {result2['call_count']}"
    ...
    assert duration3 > duration2, f"Different key call ({duration3:.2f}s) should be slower than cached call ({duration2:.2f}s)"
    
    # Verify that the call count increased
    assert result3["call_count"] > result2["call_count"], f"Call count should increase: {result3['call_count']} <= {result2['call_count']}"
    ...
    assert duration4 > duration2, f"Call after clearing cache ({duration4:.2f}s) should be slower than cached call ({duration2:.2f}s)"
    
    # Verify that the call count increased
    assert result4["call_count"] > result2["call_count"], f"Call count should increase: {result4['call_count']} <= {result2['call_count']}"
